...
public class problem8 {
    public static void main(String[] args){
        ...
        int N = Integer.parseInt(input_arr[0]); // 학생 수
        int C = Integer.parseInt(input_arr[1]); // 한줄에 앉을 수 있는 자리수

        int height[] = new int [N];
        for(int i=2;i<input_arr.length;i++){ // 학생들 키 입력
            height[i-2] = Integer.parseInt(input_arr[i]);
        }
